...
export default function PublicLiveDashboardPage() {
  ...
  const finalCountdownPlayedRef = useRef(false);
  ...
  function playRawSound(src, volume = 0.85) {
    try {
      const audio = new Audio(src);
      audio.volume = volume;

      audio.play().catch(() => {
        console.warn("Sound blocked until first browser interaction.");
      });
    } catch (error) {
  ...
  function tryAutoUnlockSound() {
    if (soundUnlockedRef.current) return;

    try {
      const audio = new Audio("/sounds/countdown-beep.mp3");
      audio.volume = 0.01;
      ...
      audio
        ...
        .catch(() => {
          // Browser blocked autoplay. First click/touch/key will unlock.
        });
    } catch {}
  }
  ...
  function unlockSound() {
    ...
    if (pendingStartSoundRef.current && !startSoundPlayedRef.current) {
      pendingStartSoundRef.current = false;
      startSoundPlayedRef.current = true;
      playRawSound("/sounds/countdown-beep.mp3", 0.9);
    }
  }
  ...
  async function loadData() {
    try {
      ...
      if (result.success) {
        ...
        setErrorMessage(result.message || "Could not load live dashboard.");
        console.error(result.message);
      }
    } catch (error) {
      setErrorMessage(error.message || "Could not load live dashboard.");
      console.warn("Live dashboard fetch skipped:", error.message);
    }
  }
  ...
  useEffect(() => {
    ...
    finalCountdownPlayedRef.current = false;
    pendingStartSoundRef.current = false;

    function updateTimer() {
      const startTime = data?.settings?.start_time;
      const endTime = data?.settings?.end_time;

      const now = getServerNowMs();
      const result = calculateTimer(startTime, endTime, now);

      const start = getTimeMs(startTime);
      const end = getTimeMs(endTime);

      const secondsBeforeStart = Number.isNaN(start)
        ? null
        : Math.ceil((start - now) / 1000);

      const secondsLeft = Number.isNaN(end)
        ? null
      ...
      if (
        ...
        !startSoundPlayedRef.current
      ) {
        const played = playSound("/sounds/countdown-beep.mp3", 0.9);
      ...
      if (
        ...
        secondsLeft <= 30 &&
        secondsLeft > 0 &&
        !finalCountdownPlayedRef.current
      ) {
        const played = playSound("/sounds/final-countdown.mp3", 0.9);

        if (played) {
          finalCountdownPlayedRef.current = true;
        }
      }
    ...
    <main className="cyber-noise min-h-screen bg-[#050505] text-white">
      ...
      <section className="cyber-grid-bg relative min-h-[calc(100vh-132px)] overflow-hidden px-3 pb-4 pt-0 md:px-4">
        ...
        <div className="relative mx-auto w-full max-w-none">
          <div
            className={`relative mx-auto mb-4 w-full max-w-[95vw] overflow-hidden rounded-b-[2rem] border border-t-0 px-5 py-5 text-center shadow-[0_0_35px_rgba(255,75,0,0.16)] ${
              timer.phase === "ended"
              ...
              <table className="w-full min-w-[1300px] border-collapse">
                <thead>
                  <tr className="border-b border-[#ff4b00]/25 bg-black/95 text-left text-[11px] uppercase tracking-[0.18em] text-slate-400">
                    ...
                    <th className="px-4 py-3">Penalty Min</th>
                  ...
                  {teams.map((team, index) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-4 py-2.5 text-2xl font-black text-[#ff4b00]">
          ...
          <div className="mt-2 flex flex-wrap gap-2 text-xs font-bold">
            ...
            <div className="rounded-full border border-[#ff4b00]/50 bg-[#ff4b00]/10 px-3 py-1.5 text-orange-200">
              Penalty = total solve time in minutes
            </div>
